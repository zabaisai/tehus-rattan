...
export class AnalyticsController {
  ...
  @Get('leads-by-stage')
  getLeadsByStage(@Request() req: any, @Query('pipelineId') pipelineId?: string) {
    return this.analyticsService.getLeadsByStage(req.user.companyId, pipelineId);
  }
  ...
  async getOverdueTasksCount(@Request() req: any) {
    const count = await this.analyticsService.getOverdueTasksCount(req.user.companyId);
    return { count };
  }

  @Get('conversations-pending')
  async getPendingConversationsCount(@Request() req: any) {
    const count = await this.analyticsService.getPendingConversationsCount(req.user.companyId);
    return { count };
  }
}